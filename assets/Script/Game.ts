...
export default class Game extends cc.Component {
     ...
     * 滑动
     * 
     * 滑动的速度为0.1s一格,计算相对距离,然后处理动作
     */
    slideSquare(isMove = false) {
        //判断游戏是否该结束
   ...
   */
    ...
    randomSqrtInNULL() {
        ...
        cc.log("nullList===>>>", nullList.length);
        cc.log("index===>>>", index);
    ...
    calcHorizontalSlidePosition(line: number) {
        ...
        for (let i = 0; i < flagList.length; i++) {
            ...
            if (this._direction == direction.left) {
                ...
                // oldNode = lineList[flagList.length - i - 1];
                // cloneNode = cc.instantiate(oldNode);
            ...
            // targetNode = lineList[i];
            let moveAct = cc.moveTo(0.3, targetNode.position);
